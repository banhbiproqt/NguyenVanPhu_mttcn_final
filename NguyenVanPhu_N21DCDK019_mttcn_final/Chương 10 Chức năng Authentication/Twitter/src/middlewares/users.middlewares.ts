      ...
      custom:{
        options: async (value: string,{req}) =>{
          if(!value){
            ...
            try {
              const decoded_email_verify_token =
              await verifyToken({ token: value, secretOrPublicKey: process.env.JWT_SECRET_EMAIL_VERIFY_TOKEN as string })
...
export const verifiedUserValidator = (req: Request, res: Response, next: NextFunction) => {
  ...
  if(verify != UserVerifySatus.Verified){
  ...
  checkSchema(
    {
      ...
      username: {
        optional: true,
        isString: {
          errorMessage: USERS_MESSAGES.USERSNAME_MUST_BE_STRING
        },
        trim: true,
        custom:{
          options: async (value: string, {req}) => {
            if(!REGEX_USERNAME.test(value)) {
              throw Error(USERS_MESSAGES.USERSNAME_INVALID)
            }
            const user = await databaseService.users.findOne({username: value})
            // Nếu đã tồn tại username này trong db
            // thì chúng ta ko cho phép update
            if(user){
