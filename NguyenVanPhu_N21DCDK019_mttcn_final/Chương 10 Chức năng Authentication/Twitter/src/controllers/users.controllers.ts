...
import { UserVerifySatus } from '~/constants/enums'
import HTTP_STATUS from '~/constants/httpStatus'
import { USERS_MESSAGES } from '~/constants/messages'
import { ChangePasswordReqBody, FollowReqBody, ForgotPasswordReqBody, GetProfileReqParams, LoginReqBody, LogoutReqBody, RegisterReqBody, ResetPasswordReqBody, TokenPayload, UnfollowReqParams, UpdateMeReqBody, VerifyEmailReqBody, VerifyForgotPasswordReqBody } from '~/models/requests/User.requests'
...
export const oauthController = async (req:Request, res: Response) => {
  ...
  const urlRedirect = `${process.env.CLIENT_REDIRECT_CALLBACK}?access_token=${result.
  access_token}&refresh_token=${result.refresh_token}&new_user=${result.newUser}`
...
export const verifyEmailController = async (req: Request<ParamsDictionary, any, VerifyEmailReqBody>, res: Response, next: NextFunction) => {
  ...
  // Đã verify rồi thì ko báo lỗi
  // Mà trả về status OK với mesage là đã verify trc đó rồi
...
export const resend = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if(user.verify == UserVerifySatus.Verified){
    return res.json({
      message: USERS_MESSAGES.EMAIL_ALREADY_VERIFIED_BEFORE
    })
  }
